...
export default class OpenAIController {
    ...
    async getDiagnosticSummary(req: Request, res: Response) {
        try {
            ...
            const run: any = await client.beta.threads.createAndRun({
                assistant_id: process.env.OPENAI_SUMMARY_ASSISTANT_ID ?? "",
                thread: {
            ...
            res.json({ status: true, data: JSON.parse(reply) });
    ...
    async getPartDetails(req: Request, res: Response) {
        try {
            const userMessage: string = JSON.stringify(req.body.message);
    ...
    async getDriveCycle(req: Request, res: Response) {
        try {
            const userMessage: string = JSON.stringify(req.body.message);
